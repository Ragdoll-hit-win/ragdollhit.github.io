...
'use strict';
{
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        ...
        _OnKeyEvent(name, e) {
            ...
            if (this._iRuntime.GetExportType() === "windows-webview2") {
...
;'use strict';
{
    const DOM_COMPONENT_ID = "file-chooser";
    function StopPropagation(e) {
        e.stopPropagation()
    }
    const HANDLER_CLASS = class FileChooserDOMHandler extends self.DOMElementHandler {
        constructor(iRuntime) {
            super(iRuntime, DOM_COMPONENT_ID);
            this.AddDOMElementMessageHandler("click", (elem,e)=>this._Click(elem, e));
            this.AddDOMElementMessageHandler("clear", elem=>this._Clear(elem))
        }
        CreateElement(elementId, e) {
            ...
            elem.addEventListener("mousedown", StopPropagation);
            elem.addEventListener("mouseup", StopPropagation);
            elem.addEventListener("change", ()=>this.PostToRuntimeElement("change", elementId, {
                "files": Array.from(elem.files)
            }));
            if (e["id"])
                elem.id = e["id"];
            if (e["className"])
        ...
        UpdateState(elem, e) {
            elem.setAttribute("accept", e["accept"]);
            if (e["isMultiSelect"])
...
'use strict';
{
    ...
    const HANDLER_CLASS = class ShareHandler extends self.DOMHandler {
        constructor(iRuntime) {
            ...
            this._isFilesSupported = IsWebShareFilesSupported() || hasCordovaPlugin;
        ...
        _OnRateApp(e) {
            ...
            if (plugin)
                plugin["Rate"](body, confirm, cancel, appID)
        }
        _OnShowStore(e) {
            const appID = e["appID"];
            const plugin = this._GetRatePlugin();
